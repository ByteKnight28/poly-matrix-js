...
function pkeKeyGen(d, paramSet = 'ML-KEM-512') {
  ...
  for (let i = 0; i < k; i++) {
    ...
    for (let j = 0; j < k; j++) {
      const temp = new Int32Array_(256);
  ...
  const ek = new Uint8Array_(384 * k + 32);
  const dk = new Uint8Array_(384 * k);
...
function pkeEncrypt(ek, m, r, paramSet = 'ML-KEM-512') {
  ...
  const vPoly = new Int32Array_(256);
  for (let j = 0; j < k; j++) {
    const temp = new Int32Array_(256);
  ...
  const c1 = new Uint8Array_(c1Length);
  for (let i = 0; i < k; i++) {
    let uPolyi = u.getPoly(i, 0);
    const compU = new Int32Array_(256);
  ...
  const c2 = new Uint8Array_(c2Length);
  const compV = new Int32Array_(256);
  for (let c=0; c<256; c++) compV[c] = compress(vPoly[c], dv);
  c2.set(byteEncode(dv, compV), 0);
  
  const cBytes = new Uint8Array_(c1Length + c2Length);
  cBytes.set(c1, 0);
...
function pkeDecrypt(dk, cBytes, paramSet = 'ML-KEM-512') {
  ...
  const sT_u = new Int32Array_(256);
  for (let i = 0; i < k; i++) {
    const temp = new Int32Array_(256);
  ...
  const w = new Int32Array_(256);
  polySub(vPoly, sT_u, w);

  const mBits = new Int32Array_(256);
  for (let i = 0; i < 256; i++) {
