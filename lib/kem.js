...
function mlKemKeyGen(d, z, paramSet = 'ML-KEM-512') {
  if (!PARAMETERS[paramSet]) throw new Error(`Unknown parameter set: ${paramSet}`);
  if (!isUint8Array(d) || d.length !== 32) throw new Error('d must be 32 bytes');
  if (!isUint8Array(z) || z.length !== 32) throw new Error('z must be 32 bytes');
  const { ek, dk } = pkeKeyGen(d, paramSet);
  
  const fullDk = new Uint8Array_(dk.length + ek.length + 32 + 32);
  fullDk.set(dk, 0);
...
function mlKemEncaps(ek, m, paramSet = 'ML-KEM-512') {
  if (!PARAMETERS[paramSet]) throw new Error(`Unknown parameter set: ${paramSet}`);
  if (!isUint8Array(m) || m.length !== 32) throw new Error('m must be 32 bytes');
  const k = PARAMETERS[paramSet].k;
  if (!isUint8Array(ek) || ek.length !== 384 * k + 32) {
    throw new Error(`ek must be ${384 * k + 32} bytes for ${paramSet}`);
  }
  const input = new Uint8Array_(32 + 32);
  input.set(m, 0);
  input.set(H(ek), 32);
...
function mlKemDecaps(c, dk, paramSet = 'ML-KEM-512') {
  if (!PARAMETERS[paramSet]) throw new Error(`Unknown parameter set: ${paramSet}`);
  const kLength = PARAMETERS[paramSet].k;
  const dkPkeLen = 384 * kLength;
  const ekPkeLen = 384 * kLength + 32;
  const expectedDkLen = dkPkeLen + ekPkeLen + 32 + 32;
  if (!isUint8Array(dk) || dk.length !== expectedDkLen) {
    throw new Error(`dk must be ${expectedDkLen} bytes for ${paramSet}`);
  }
  ...
  // Constant-time boolean mask: if fail > 0, mask is 0xFF; if fail === 0, mask is 0.
  // We use `>>> 31` to extract the sign bit after setting it with logical OR.
  fail = ((fail | -fail) >>> 31) * 0xFF;
  
  const finalK = new Uint8Array_(32);
  for (let i = 0; i < 32; i++) {
